...
const AddTeacher = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        if (isFormDirty === false) {
            ...
            try {
                const response = await axios.post('http://localhost:3500/teachers/new', {
                    data: data
                });
                ...
                if (response.data.statusCode === 200 && response.data.success === false) {
                    ...
                    setTransition(TransitionUp);
                    setOpen(true);
                    setSnackbarSuccessMsg("Teacher record created successfully!");
                } else if (response.data.statusCode === 500 && response.data.success === false) {
                    setTransition(TransitionUp);
                    setErrorOpen(true);
                    setSnackbarFailMsg('Error creating teacher record!');
                }
        ...
        <>
            <div className="dashboard-index">
                ...
                <div className="content">
                    <Navbar title='Add New Teacher' landingPage={false} />
                                    ...
                                    <FormControl required className='form-control'>
                                        ...
                                        >
                                            {departmentData.length > 0 ?
                                                ...
                                                :
                                                (
                                                    <MenuItem key={data.department} value={data.department}>No department exits</MenuItem>
                                                )
                                            }
                                        </Select>
                                    </FormControl>
