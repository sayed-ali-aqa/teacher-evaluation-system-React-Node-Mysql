...
const AddStudent = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        if (!data.stu_reg_id || data.stu_reg_id.trim() === '') {
            ...
            isFormDirty = true;
        } else if (data.stu_reg_id.length > 15) {
            setStuIDErrMsg('Regitration ID should be 15 chracters');
            isFormDirty = true;
        } else {
            setStuIDErrMsg('');
        }
        ...
        if (!data.name || data.name.trim() === '') {
            ...
            setNameErrMsg('Name should be 50 chracters');
            isFormDirty = true;
        } else {
            setNameErrMsg('');
        }
        ...
        if (!isFormDirty) {
            ...
            try {
                const response = await axios.post('http://localhost:3500/students/new', {
                    data: data
                });
                ...
                if (response.data.statusCode === 200 && response.data.success === false) {
                    ...
                    setTransition(TransitionUp);
                    setOpen(true);
                    setSnackbarSuccessMsg("Student record created successfully!");
                }
                else if (response.data.statusCode === 500 && response.data.success === false) {
                    setTransition(TransitionUp);
                    setErrorOpen(true);
                    setSnackbarFailMsg('Error creating student record!');
                }
        ...
        <>
            <div className="dashboard-index">
                ...
                <div className="content">
                    <Navbar title='Add New Student' landingPage={false} />
